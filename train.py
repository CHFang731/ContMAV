...
def train_main():
    ...
    # === 修改開始: 防止 epochs 為 0 導致 OneCycleLR 崩潰 ===
    print(f"[DEBUG] args.epochs before scheduler: {args.epochs}")
    
    # 強制確保 total_steps 至少為 1
    if args.epochs <= 0:
        print("[WARNING] args.epochs is 0 or negative. Forcing it to 1 to prevent crash.")
        scheduler_steps = 1
    else:
        scheduler_steps = args.epochs
